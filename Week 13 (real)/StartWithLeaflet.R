...
# Answer: It is longitude first and then latitude 
...
#Answer: T creates a combined display of the map with all the layers 
# in the last line of code above with F?
# Answer: F turns the map into the 3 different displays 
...
l_dk <- leaflet() %>%   # assign the base location to an object
  setView( 10.2040287,56.1708646, zoom = 13) %>% 
  addTiles()
l_dk
...
# Answer: The rows seem to have 14 of them that have missing or invalid values 
studentmap<- leaflet() %>% 
  addTiles() %>% 
...
for (provider in esri) {
  studentmap <- studentmap %>% addProviderTiles(provider, group = provider)
}
...
# Solution
# Alternative: Starting from your studentmap
DANmap <- studentmap %>%
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste(places$Description, "<br>", places$Type),
...
# Your brief answer
#The clustering option gives a better look at the total amount, without having to count it yourself. It also creates a less messy map. However, when removing the clustering you can see without zooming in that markers are also there in the western part of Jutland (As an example). While clustering you have to zoom in concretely to get this visualisation.  
...
# Solution
# Assuming your places dataframe has columns: Notes and Classifications
DANmap_with_details <- DANmap %>%
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste("Description:", places$Description, "<br>",
                           "Type:", places$Type, "<br>",
                           "Classification:", places$Classifications),
             clusterOptions = markerClusterOptions())

DANmap <- DANmap_with_details
